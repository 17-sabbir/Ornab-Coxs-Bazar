...
.uerd-tabs .nav-link { color: var(--brand-text); opacity: 0.6; }
.uerd-tabs .nav-link.active { color: var(--brand-navy) !important; border-bottom: 2px solid var(--brand-coral) !important; background: transparent !important; }
</style>
<div class="container py-5">
    <div class="row">
        <div class="col-12 text-center mb-5">
            <h2 class="display-5 fw-bold uerd-page-title">Our Development Journey</h2>
            <p class="lead uerd-page-subtitle">Explore a selection of our current and past projects, showcasing our ideas, innovation, and impact.</p>
        </div>
    </div>

    <!-- Tabs for Filtering -->
    <ul class="nav nav-pills mb-4 justify-content-center uerd-tabs" id="pills-tab" role="tablist">
        ...
        <div class="tab-pane fade show active" id="pills-all" role="tabpanel">
            <div class="row g-4">
                ...
                <div class="col-lg-4 col-md-6">
                    <div class="card h-100 shadow-sm border-0 uerd-project-card" style="border-top: 4px solid {{ $projectStatus == 'ongoing' ? 'var(--brand-green)' : 'var(--brand-teal)' }};">
                        {{-- Image Section (Top) --}}
                        <div style="height: 220px; overflow: hidden; position: relative;">
                            ...
                            @else
                                <div class="w-100 h-100 bg-light d-flex align-items-center justify-content-center">
                                    <i class="fa-regular fa-folder-open fa-3x uerd-body-muted opacity-25"></i>
                                </div>
                            @endif
                            <span class="position-absolute top-0 end-0 m-3 badge {{ $projectStatus == 'ongoing' ? 'uerd-status-ongoing' : 'uerd-status-completed' }}">{{ ucfirst($project->status) }}</span>
                        </div>

                        <div class="card-body d-flex flex-column">
                            <h5 class="card-title fw-bold uerd-page-title mb-3" style="min-height: 48px;">{{Str::limit($project->project_name, 60)}}</h5>
                            
                            <h6 class="uerd-objectives fw-bold border-bottom pb-2 mb-3" style="font-size: 0.9rem;">Objectives</h6>
                            ...
                            <a href="{{ route('ongoing.project.view', $project->id) }}" class="uerd-read-more w-100 mt-auto rounded-pill justify-content-center">
        ...
        <div class="tab-pane fade" id="pills-ongoing" role="tabpanel">
             <div class="row g-4">
                @foreach($projects->filter(function($project){ return strtolower($project->status) == 'ongoing'; }) as $project)
                 <div class="col-lg-4 col-md-6">
                     <div class="card h-100 shadow-sm border-0 uerd-project-card" style="border-top: 4px solid var(--brand-green);">
                        {{-- Image Section (Top) --}}
                        <div style="height: 220px; overflow: hidden; position: relative;">
                            @if(!empty($project->image))
                                <img src="{{ asset('images/project/'.$project->image) }}" class="card-img-top w-100 h-100" alt="{{ $project->project_name }}" style="object-fit: cover;">
                            ...
                            @endif
                            <span class="position-absolute top-0 end-0 m-3 badge uerd-status-ongoing">{{ ucfirst($project->status) }}</span>
                        </div>
                 ...
                 <div class="col-lg-4 col-md-6">
                     <div class="card h-100 shadow-sm border-0 uerd-project-card" style="border-top: 4px solid var(--brand-teal);">
                        ...
                        <div style="height: 220px; overflow: hidden; position: relative;">
                            ...
                            @endif
                            <span class="position-absolute top-0 end-0 m-3 badge uerd-status-completed">{{ ucfirst($project->status) }}</span>
                        </div>
